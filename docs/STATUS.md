...
| gold_test | vẫn RỖNG (chưa có nhãn gold chính thức). **21/09: hạ tầng xong** — 1.601 dòng sự kiện/367 file audioset_strong đã vào `splits.csv` với `split=gold_test`. **Pilot lần 1 (bước 1 của §8.3) đã gán xong 25/30 file** — **12 file đã loại tính đến 22/09** (6 "rác" `unusable` + 6 game/phim `synthetic_or_game_audio`, `select_gold_pilot.py` chọn thay thế mỗi lần). **Phát hiện mẫu hình 100%: cả 12 file bị loại đều chạm ít nhất một trong 5 lớp glass_breaking/scream/explosion/gunshot/siren** — súng nổ/bom/kính vỡ/tiếng thét thật hiếm khi được quay đăng YouTube, nên phần lớn nhãn AudioSet cho các lớp này là game/phim. Pool chưa cạn (`scream` còn 19/22) nhưng các lớp này sẽ tốn nhiều lượt nghe hơn hẳn để đủ ≥20 sự kiện/lớp ở cổng §8.5. Quy tắc kiểm tra mới ở `annotation_guideline.md` §0.1, phân tích đầy đủ ở `data/gold/decision_log.md`. Còn thiếu: gán 5 file thay thế mới nhất, rồi mới đủ 30 để làm bước 2 |
