...
| gold_test | vẫn RỖNG (chưa có nhãn gold chính thức). **21/09: hạ tầng xong** — 1.601 dòng sự kiện/367 file audioset_strong đã vào `splits.csv` với `split=gold_test`. **Pilot lần 1 (bước 1 của §8.3) đã gán xong 24/30 file** — 6 file "rác/không liên quan" (`reason_code=unusable`) + 1 file phát hiện là **hiệu ứng game, không phải audio thật** (`reason_code=synthetic_or_game_audio`, rủi ro mới, có thể lặp lại) đã loại khỏi `gold_test`, `select_gold_pilot.py` chọn thay thế. Quy tắc mới ở `annotation_guideline.md` §0.1. Còn thiếu: gán 6 file thay thế, rồi mới đủ 30 để làm bước 2 (nghỉ ≥3 ngày, gán lại mù) |
