...
| gold_test | vẫn RỖNG (chưa có nhãn gold chính thức). **21/09: hạ tầng xong** — 1.601 dòng sự kiện/367 file audioset_strong đã vào `splits.csv` với `split=gold_test`. **Pilot lần 1 (bước 1 của §8.3) đã gán xong 24/30 file** — 6 file bị đánh giá "rác/không liên quan" đã ghi vào `exclusions.csv` (stage `gold_pilot`, reason `unusable`) và bị loại khỏi `gold_test` luôn; `select_gold_pilot.py` đã chọn 6 file thay thế. Còn thiếu: gán 6 file thay thế, rồi mới đủ 30 để làm bước 2 (nghỉ ≥3 ngày, gán lại mù) |
